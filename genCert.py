...
"""
    ...
    Then, create the ca.cert file with:
        ...
        UPDATE: for chrome > 39: openssl req -new -x509 -sha256 -days 3650 -key ca.key -out ca.cert
        double click on cert inUbuntu -- > import
        Chrome setting--> Manage SSL certificate--> Authorities--> import 
...
X509_EXTRA_ARGS = ('-passin', 'pass:nrgwings')
...
def gencert(domain, rootdir=MYDIR, keysize=KEY_SIZE, days=DAYS,
    ...
    openssl('x509', '-req', '-days', str(days), '-in', dfile('request'),
            ...
            '0x%s' % hashlib.md5((domain + 
                                 str(datetime.datetime.now())).encode("utf8")).hexdigest(),
            '-out', dfile('cert'),
